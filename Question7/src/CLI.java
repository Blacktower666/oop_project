...
public class CLI {
	...
	public static void main(String[] args) throws UnknownConsumerException, NotAPowerGeneratorException {
		...
		while (true) {
			int num1 = readPosInt("Type an action (total:1 add:2 get:3 more:4 less:5 quit:6):");// read the number from
																								// user
			switch (num1) {
			case 1:
				System.out.println("Total amount of power consumed:" + ele.totalConsumption());// pritn out the sum of
																								// power
				break;
			case 2:
				...
				if (num2 < 1 || num2 > 3) {// judge which consumer should be added
					...
					case 3:
						...
						System.out
								.println("Solor Houese \"" + name + "\"generating " + power + " watts of power added");
